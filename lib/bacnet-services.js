...
var encodeWritePropertyMultiple = module.exports.encodeWritePropertyMultiple = function(buffer, objectId, valueList) {
  ...
  valueList.forEach(function(pValue) {
    ...
    pValue.values.forEach(function(value) {
      baAsn1.bacappEncodeApplicationData(buffer, value);
    });
...
module.exports.encodeSubscribeCOV = function(buffer, subscriberId, objectId, isCancellation, isConfirmed, lifetime) {
  baAsn1.encodeContextUnsigned(buffer, 0, subscriberId);
  baAsn1.encodeContextObjectId(buffer, 1, objectId.type, objectId.instance);
  if (!isCancellation) {
    baAsn1.encodeContextBoolean(buffer, 2, isConfirmed);
...
module.exports.decodeSubscribeCOV = function(buffer, offset, apduLen) {
  ...
  decodedValue = baAsn1.decodeUnsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  value.subscriberId = decodedValue.value;
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 1)) return;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  ...
  if (len < apduLen) {
    ...
    if (baAsn1.decodeIsContextTag(buffer, offset + len, 2)) {
      ...
      result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
      len += result.len;
      value.isConfirmed = buffer[offset + len] > 0;
      len++;
    }
    value.lifetime = 0;
...
module.exports.encodeSubscribeProperty = function(buffer, subscriberId, objectId, isCancellation, isConfirmed, lifetime, property, covIncrementPresent, covIncrement) {
  baAsn1.encodeContextUnsigned(buffer, 0, subscriberId);
  baAsn1.encodeContextObjectId(buffer, 1, objectId.type, objectId.instance);
  if (!isCancellation) {
    baAsn1.encodeContextBoolean(buffer, 2, isConfirmed);
  ...
  baAsn1.encodeContextEnumerated(buffer, 0, property.id);
  if (property.index !== baAsn1.BACNET_ARRAY_ALL) {
    baAsn1.encodeContextUnsigned(buffer, 1, property.index);
  }
...
module.exports.encodeEventNotifyData = function(buffer, data) {
  ...
  baAsn1.encodeContextObjectId(buffer, 1, data.initObjectId.type, data.initObjectId.instance);
    ...
    case baEnum.BacnetNotifyTypes.NOTIFY_EVENT:
      ...
      switch (data.eventType) {
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_BITSTRING:
          baAsn1.encodeOpeningTag(buffer, 0);
          baAsn1.encodeContextBitstring(buffer, 0, data.bitstringChange.bitstring);
          baAsn1.encodeContextBitstring(buffer, 1, data.bitstringChange.statusFlags);
          baAsn1.encodeClosingTag(buffer, 0);
          break;
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_STATE:
          baAsn1.encodeOpeningTag(buffer, 1);
          baAsn1.encodeOpeningTag(buffer, 0);
          baAsn1.bacappEncodePropertyState(buffer, data.stateChange.state);
          baAsn1.encodeClosingTag(buffer, 0);
          baAsn1.encodeContextBitstring(buffer, 1, data.stateChange.statusFlags);
          baAsn1.encodeClosingTag(buffer, 1);
          break;
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_VALUE:
          ...
          switch (data.changeOfValueTag) {
            case baEnum.BacnetCOVTypes.CHANGE_OF_VALUE_REAL:
              baAsn1.encodeContextReal(buffer, 1, data.valueChange.value);
              break;
            case baEnum.BacnetCOVTypes.CHANGE_OF_VALUE_BITS:
              baAsn1.encodeContextBitstring(buffer, 0, data.valueChange.value);
              break;
            default:
              throw new Error('NotImplemented');
          }
          baAsn1.encodeClosingTag(buffer, 0);
          baAsn1.encodeContextBitstring(buffer, 1, data.valueChange.statusFlags);
          baAsn1.encodeClosingTag(buffer, 2);
          break;
        case baEnum.BacnetEventTypes.EVENT_FLOATING_LIMIT:
          baAsn1.encodeOpeningTag(buffer, 4);
          baAsn1.encodeContextReal(buffer, 0, data.floatingLimit.value);
          baAsn1.encodeContextBitstring(buffer, 1, data.floatingLimit.statusFlags);
          baAsn1.encodeContextReal(buffer, 2, data.floatingLimit.setpoint);
          baAsn1.encodeContextReal(buffer, 3, data.floatingLimit.limit);
          baAsn1.encodeClosingTag(buffer, 4);
          break;
        case baEnum.BacnetEventTypes.EVENT_OUT_OF_RANGE:
          baAsn1.encodeOpeningTag(buffer, 5);
          baAsn1.encodeContextReal(buffer, 0, data.outOfRange.value);
          baAsn1.encodeContextBitstring(buffer, 1, data.outOfRange.statusFlags);
          baAsn1.encodeContextReal(buffer, 2, data.outOfRange.deadband);
          baAsn1.encodeContextReal(buffer, 3, data.outOfRange.limit);
          baAsn1.encodeClosingTag(buffer, 5);
          break;
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_LIFE_SAFETY:
          baAsn1.encodeOpeningTag(buffer, 8);
          baAsn1.encodeContextEnumerated(buffer, 0, data.lifesafetyChange.state);
          baAsn1.encodeContextEnumerated(buffer, 1, data.lifesafetyChange.mode);
          baAsn1.encodeContextBitstring(buffer, 2, data.lifesafetyChange.statusFlags);
          baAsn1.encodeContextEnumerated(buffer, 3, data.lifesafetyChange.operation);
        ...
        case baEnum.BacnetEventTypes.EVENT_BUFFER_READY:
          ...
          baAsn1.encodeContextUnsigned(buffer, 1, data.bufferReadyBufferProperty.previousNotification);
          baAsn1.encodeContextUnsigned(buffer, 2, data.bufferReadyBufferProperty.currentNotification);
          baAsn1.encodeClosingTag(buffer, 10);
          break;
        case baEnum.BacnetEventTypes.EVENT_UNSIGNED_RANGE:
          baAsn1.encodeOpeningTag(buffer, 11);
          baAsn1.encodeContextUnsigned(buffer, 0, data.unsignedRange.value);
          baAsn1.encodeContextBitstring(buffer, 1, data.unsignedRange.statusFlags);
          baAsn1.encodeContextUnsigned(buffer, 2, data.unsignedRange.limit);
...
module.exports.decodeEventNotifyData = function(buffer, offset) {
  ...
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  len += decodedValue.len;
  eventData.initObjectId = {type: decodedValue.objectType, instance: decodedValue.instance};
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 2)) return;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
...
module.exports.decodeDeleteObject = function(buffer, offset, apduLen) {
  ...
  var len = 1;
  var decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  var value = {type: decodedValue.objectType, instance: decodedValue.instance, len: decodedValue.len};
  len += value.len;
  if (len !== apduLen) return;
  value.len = len;
  return value;
...
module.exports.encodeCreateObject = function(buffer, objectId, valueList) {
  ...
  valueList.forEach(function(propertyValue) {
    ...
    baAsn1.encodeOpeningTag(buffer, 2);
    propertyValue.values.forEach(function(value) {
      baAsn1.bacappEncodeApplicationData(buffer, value);
    });
    baAsn1.encodeClosingTag(buffer, 2);
...
module.exports.encodeCOVNotify = function(buffer, subscriberId, deviceId, objectId, timeRemaining, values) {
  baAsn1.encodeContextUnsigned(buffer, 0, subscriberId);
  baAsn1.encodeContextObjectId(buffer, 1, baEnum.BacnetObjectTypes.OBJECT_DEVICE, deviceId);
  baAsn1.encodeContextObjectId(buffer, 2, objectId.type, objectId.instance);
...
module.exports.decodeCOVNotify = function(buffer, offset, apduLen) {
  ...
  decodedValue = baAsn1.decodeUnsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  var subscriberId = decodedValue.value;
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 1)) return;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  len += decodedValue.len;
  var deviceId = {type: decodedValue.objectType, instance: decodedValue.instance};
  if (!baAsn1.decodeIsContextTag(buffer, offset + len, 2)) return;
  result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decodeObjectId(buffer, offset + len);
  ...
  while ((apduLen - len) > 1 && !baAsn1.decodeIsClosingTagNumber(buffer, offset + len, 4)) {
    ...
    while ((apduLen - len) > 1 && !baAsn1.decodeIsClosingTagNumber(buffer, offset + len, 2)) {
      decodedValue = baAsn1.bacappDecodeApplicationData(buffer, offset + len, apduLen + offset, objectId.type, newEntry.property.id);
      if (!decodedValue) return;
      len += decodedValue.len;
      delete decodedValue.len;
      properties.push(decodedValue);
    }
    newEntry.values = properties;
    len++;
    if (baAsn1.decodeIsContextTag(buffer, offset + len, 3)) {
      result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
      len += result.len;
...
module.exports.encodeAlarmAcknowledge = function(buffer, ackProcessIdentifier, eventObjectId, eventStateAcked, ackSource, eventTimeStamp, ackTimeStamp) {
...
module.exports.encodeAlarmSummary = function(buffer, objectId, alarmState, acknowledgedTransitions) {
  baAsn1.encodeApplicationObjectId(buffer, objectId.type, objectId.instance);
