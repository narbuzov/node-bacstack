...
var bitstringBytesUsed = function(bitsUsed) {
  var len = 0;
  if (bitsUsed > 0) {
    var lastBit = bitsUsed - 1;
...
var bitstringOctet = function(bitString, octetIndex) {
  ...
  if (bitString) {
    if (octetIndex < MAX_BITSTRING_BYTES) {
      octet = bitString[octetIndex];
    }
  }
  return octet;
};

var encodeBitstring = function(buffer, bitString, bitsUsed) {
  if (bitsUsed === 0) {
    buffer.buffer[buffer.offset++] = 0;
  } else {
    var usedBytes = bitstringBytesUsed(bitsUsed);
    var remainingUsedBits = bitsUsed - ((usedBytes - 1) * 8);
...
var encodeApplicationBitstring = function(buffer, bitString, bitsUsed) {
  var bitStringEncodedLength = 1;
  bitStringEncodedLength += bitstringBytesUsed(bitsUsed);
  encodeTag(buffer, baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_BIT_STRING, false, bitStringEncodedLength);
  encodeBitstring(buffer, bitString, bitsUsed);
};
...
var encodeCovSubscription = function(buffer, value) {
  ...
  encodeOpeningTag(buffer, 1);
  encodeContextObjectId(buffer, 0, value.objectId.type, value.objectId.instance);
  encodeContextEnumerated(buffer, 1, value.property.id);
  if (value.property.index !== BACNET_ARRAY_ALL) {
    encodeContextUnsigned(buffer, 2, value.property.index);
  }
  encodeClosingTag(buffer, 1);
  encodeContextBoolean(buffer, 2, value.isConfirmed);
...
var bacappEncodeApplicationData = module.exports.bacappEncodeApplicationData = function(buffer, value) {
  ...
  switch (value.type) {
    ...
    case baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_BIT_STRING:
      encodeApplicationBitstring(buffer, value.value, value.bitsUsed);
      break;
...
var bacappEncodeDeviceObjPropertyRef = function(buffer, value) {
  encodeContextObjectId(buffer, 0, value.objectId.type, value.objectId.instance);
  encodeContextEnumerated(buffer, 1, value.property.id);
  if (value.index !== BACNET_ARRAY_ALL) {
    encodeContextUnsigned(buffer, 2, value.property.index);
  }
  if (value.deviceId.type === baEnum.BacnetObjectTypes.OBJECT_DEVICE) {
    encodeContextObjectId(buffer, 3, value.deviceId.type, value.deviceId.instance);
  }
};
...
var encodeContextBitstring = module.exports.encodeContextBitstring = function(buffer, tagNumber, bitString, bitsUsed) {
  var bitStringEncodedLength = 1;
  bitStringEncodedLength += bitstringBytesUsed(bitsUsed);
...
var bacappDecodeApplicationData = module.exports.bacappDecodeApplicationData = function(buffer, offset, maxOffset, objectType, propertyId) {
  if (!isContextSpecific(buffer[offset])) {
    ...
    if (result) {
      ...
      // HACK: Drop datatype specific handling ASAP
      if (result.encoding !== undefined) resObj.encoding = result.encoding;
      if (result.bitsUsed !== undefined) resObj.bitsUsed = result.bitsUsed;
      return resObj;
    }
  } else {
...
var decodeReadAccessResult = module.exports.decodeReadAccessResult = function(buffer, offset, apduLen) {
  ...
  while ((apduLen - len) > 0) {
    ...
    newEntry.property = {};
    newEntry.property.id = result.value;
    len += result.len;
    ...
    if (result.tagNumber === 3) {
      ...
      newEntry.property.index = result.value;
      len += result.len;
    } else {
      newEntry.property.index = BACNET_ARRAY_ALL;
    }
    result = decodeTagNumberAndValue(buffer, offset + len);
    len += result.len;
    if (result.tagNumber === 4) {
      ...
      while ((len + offset) <= buffer.length && !decodeIsClosingTagNumber(buffer, offset + len, 4)) {
        ...
        if (localResult.bitsUsed !== undefined) resObj.bitsUsed = localResult.bitsUsed;
        localValues.push(resObj);
      }
      if (!decodeIsClosingTagNumber(buffer, offset + len, 4)) return;
      if ((localValues.count === 2) && (localValues[0].type === baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_DATE) && (localValues[1].type === baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_TIME)) {
...
var bacappDecodeData = function(buffer, offset, maxLength, tagDataType, lenValueType) {
  ...
  switch (tagDataType) {
    ...
    case baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_BIT_STRING:
      ...
      value.len += result.len;
      value.value = result.value;
      value.bitsUsed = result.bitsUsed;
      break;
    case baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_ENUMERATED:
...
var decodeCovSubscription = function(buffer, offset, apduLen) {
  ...
  if (result.tagNumber === 2) {
    ...
    value.property.index = BACNET_ARRAY_ALL;
  }
  if (!decodeIsClosingTagNumber(buffer, offset + len, 1)) return;
  len++;
  result = decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
  if (result.tagNumber !== 2) return;
  value.isConfirmed = buffer[offset + len] > 0 ? true : false;
  len++;
  result = decodeTagNumberAndValue(buffer, offset + len);
  len += result.len;
...
var bacappEncodeContextTimestamp = module.exports.bacappEncodeContextTimestamp = function(buffer, tagNumber, value) {
  if (value.type !== baEnum.BacnetTimestampTags.TIME_STAMP_NONE) {
    encodeOpeningTag(buffer, tagNumber);
